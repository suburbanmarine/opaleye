...
jpeg_nvdec_pipe::jpeg_nvdec_pipe()
{
  
}

void jpeg_nvdec_pipe::add_to_bin(const Glib::RefPtr<Gst::Bin>& bin)
{
  bin->add(m_bin);
}
bool jpeg_nvdec_pipe::link_front(const Glib::RefPtr<Gst::Element>& node)
{
  node->link(m_in_queue);
  return false;
}
bool jpeg_nvdec_pipe::link_back(const Glib::RefPtr<Gst::Element>& node)
{
  m_out_tee->link(node);
  return true;
}

bool jpeg_nvdec_pipe::init(const char name[])
{
