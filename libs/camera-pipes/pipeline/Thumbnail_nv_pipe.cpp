...
Thumbnail_nv_pipe::Thumbnail_nv_pipe()
{
  
}

void Thumbnail_nv_pipe::add_to_bin(const Glib::RefPtr<Gst::Bin>& bin)
{
  bin->add(m_bin);
}

bool Thumbnail_nv_pipe::link_front(const Glib::RefPtr<Gst::Element>& node)
{
  node->link(m_in_queue);
  return true;
}

bool Thumbnail_nv_pipe::init(const char name[])
{
...
void Thumbnail_nv_pipe::handle_new_sample()
{
  ...
  if(sample)
  {
    ...
    SPDLOG_INFO("Thumbnail_nv_pipe::handle_new_sample has {}", buffer->get_size());
    {
      ...
      for(guint i = 0; i < num; i++)
      {
        ...
        SPDLOG_INFO("Thumbnail_nv_pipe::handle_new_sample block {} is {}", i, map_info.get_size());
  ...
  {
    SPDLOG_INFO("Thumbnail_nv_pipe::handle_new_sample has null sample"); 
  }
}
