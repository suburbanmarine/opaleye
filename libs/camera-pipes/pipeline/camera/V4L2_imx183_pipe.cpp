...
bool V4L2_imx183_pipe::init(const char name[])
{
  ...
  switch(m_fourcc)
  {
    case PIX_FMT_RG12:
    {
      m_src_caps= gst_caps_from_string("video/x-raw, format=(string)GRAY16_LE, framerate=1/1, pixel-aspect-ratio=1/1, width=5440, height=3648, interlace-mode=progressive, colorimetry=sRGB");
      break;
    }
    default:
    {
...
void V4L2_imx183_pipe::new_frame_cb_RG12(const v4l2_base::ConstMmapFramePtr& frame_buf)
{
  ...
  #if 1
  {
    ...
    gsize offset[1] = {0};
    gint xstride = frame_buf->get_fmt().fmt.pix.bytesperline;
