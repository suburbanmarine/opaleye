...
{
	QUERY_STRING      = QUERY_STRING =FCGX_GetParam("QUERY_STRING",    request->envp);
	REQUEST_METHOD    = REQUEST_METHOD =FCGX_GetParam("REQUEST_METHOD",  request->envp);
	CONTENT_TYPE      = CONTENT_TYPE =FCGX_GetParam("CONTENT_TYPE",    request->envp);
	CONTENT_LENGTH    = CONTENT_LENGTH =FCGX_GetParam("CONTENT_LENGTH",  request->envp);
	SCRIPT_NAME       = SCRIPT_NAME =FCGX_GetParam("SCRIPT_NAME",     request->envp);
	REQUEST_URI       = REQUEST_URI =FCGX_GetParam("REQUEST_URI",     request->envp);
	DOCUMENT_URI      = DOCUMENT_URI =FCGX_GetParam("DOCUMENT_URI",    request->envp);
	DOCUMENT_ROOT     = DOCUMENT_ROOT =FCGX_GetParam("DOCUMENT_ROOT",   request->envp);
	SERVER_PROTOCOL   = SERVER_PROTOCOL =FCGX_GetParam("SERVER_PROTOCOL", request->envp);
	REQUEST_SCHEME    = REQUEST_SCHEME =FCGX_GetParam("REQUEST_SCHEME",  request->envp);
	HTTPS             = FCGX_GetParam("HTTPS", request->envp);
	HTTPS             = (HTTPS) ? (HTTPS) : ("<null>");
	GATEWAY_INTERFACE = GATEWAY_INTERFACE =FCGX_GetParam("GATEWAY_INTERFACE", request->envp);
	SERVER_SOFTWARE   = SERVER_SOFTWARE =FCGX_GetParam("SERVER_SOFTWARE",   request->envp);
	REMOTE_ADDR       = REMOTE_ADDR =FCGX_GetParam("REMOTE_ADDR",       request->envp);
	REMOTE_PORT       = REMOTE_PORT =FCGX_GetParam("REMOTE_PORT",       request->envp);
	SERVER_ADDR       = SERVER_ADDR =FCGX_GetParam("SERVER_ADDR",       request->envp);
	SERVER_PORT       = SERVER_PORT =FCGX_GetParam("SERVER_PORT",       request->envp);
	SERVER_NAME       = SERVER_NAME =FCGX_GetParam("SERVER_NAME",       request->envp);
	REDIRECT_STATUS   = REDIRECT_STATUS =FCGX_GetParam("REDIRECT_STATUS",   request->envp);
...
void http_req_util::log_request_env()
{
    ...
    request_env.put<std::string>("HTTPS", (HTTPS) ? (HTTPS) : ("<null>"));
