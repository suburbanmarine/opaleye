...
docker buildx build --cache-to=type=local,dest=../cache --cache-from=type=local,src=../cache --platform linux/arm64 --output=type=image,push=true -t suburbanmarine/opaleye-ci:l4t-r32.6.1 .

CONTAINER_ID=$(docker create --rm -it suburbanmarine/opaleye-ci:l4t-r32.6.1 /bin/bash)
