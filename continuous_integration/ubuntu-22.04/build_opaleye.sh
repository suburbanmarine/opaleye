...
docker buildx build --cache-to=type=local,dest=cache --cache-from=type=local,src=cache --platform linux/amd64 --output=type=local,dest=opaleye-ci_22.04.tar.gz -t suburbanmarine/opaleye-ci:22.04 .

CONTAINER_ID=$(docker create -it suburbanmarine/opaleye-ci:22.04 /bin/bash)
