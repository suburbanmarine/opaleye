...
// CAM1_GPIO F7 - trigger_to_sensor - GPIO3_PN.02

// J140 CD
// CAM0_MCLK F9 - flash_from_sensor - GPIO3_PO.00
// CAM0_GPIO G8 - trigger_to_sensor - GPIO3_PR.00

// J140 EF
// CAM2_MCLK E7 - flash_from_sensor - GPIO3_PN.01
// CAM2_GPIO H7 - trigger_to_sensor - GPIO3_PR.01
...
typedef struct opaleye_gpio_state_t
{
	...
	struct gpio csi_gpo[3];
	struct gpio csi_gpi[3];
...
int __init opaleye_gpio_init(void)
{
	...
	state->csi_gpo[0].gpio  = get_tegra_gpio(TEGRA186_BASE, TEGRA_MAIN_GPIO_PORT_N, 2);
	...
	state->csi_gpo[1].gpio  = get_tegra_gpio(TEGRA186_BASE, TEGRA_MAIN_GPIO_PORT_R, 0);
	...
	state->csi_gpo[2].gpio  = get_tegra_gpio(TEGRA186_BASE, TEGRA_MAIN_GPIO_PORT_R, 1);
	state->csi_gpo[2].flags = GPIOF_DIR_OUT | GPIOF_INIT_LOW;
	state->csi_gpo[2].label = "opaleye_GPIO03_R01";

	int ret = gpio_request_array(state->csi_gpo, sizeof(state->csi_gpo) / sizeof(state->csi_gpo[0]));
	if(ret != 0)
	{
		printk(KERN_ERR "gpio_request_one failed to start csi_gpo");
		kfree(state);
		state = NULL;
		return -1;
	}
	...
	state->csi_gpi[0].gpio  = get_tegra_gpio(TEGRA186_BASE, TEGRA_MAIN_GPIO_PORT_O, 1);
	...
	if( ! state->opaleye_dir_kobj )
	{
		...
		gpio_free_array(state->csi_gpi, sizeof(state->csi_gpi) / sizeof(state->csi_gpi[0]));
